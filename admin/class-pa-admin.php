...
class Admin
{
	...
	public function add_answer_ajax_handler()
	{
		...
		<div class="answer-item" data-answer-id="<?php echo esc_attr($answer->id); ?>">
			...
			<input type="text" class="answer-label-input" value=""
				placeholder="<?php esc_attr_e('Answer Label', 'personality-assessment'); ?>" />
			...
			<button class="button delete-answer-button" data-answer-id="<?php echo esc_attr($answer->id); ?>"
				data-nonce="<?php echo esc_attr(wp_create_nonce('pa_delete_answer_' . $answer->id)); ?>"><span
	...
	public function render_question_item($question)
	{
		...
		<div class="question-item" data-question-id="<?php echo esc_attr($question->id); ?>"
			...
			data-is-required="<?php echo esc_attr($question->is_required); ?>">
			...
			<div class="question-title-header">
				...
				<div class="question-actions">
					...
					<a href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=personality-assessment&action=delete_question&question_id=' . $question->id), 'pa_delete_question')); ?>"
						class="button delete-question"><span class="dashicons dashicons-trash"></span></a>
				</div>
			</div>
			...
			<div class="answers-section">
					...
					if ($answers) {
						foreach ($answers as $answer) {
							...
							<div class="answer-item" data-answer-id="<?php echo esc_attr($answer->id); ?>">
									...
									if (is_array($label_weights)) {
										foreach ($label_weights as $label => $weight) {
											...
											<div class="label-weight-item">
												<input type="text" class="answer-personality-label-input"
													value="<?php echo esc_attr($label); ?>"
													placeholder="<?php esc_attr_e('Label', 'personality-assessment'); ?>" />
												<input type="number" class="answer-weight-input" value="<?php echo esc_attr($weight); ?>"
													placeholder="<?php esc_attr_e('Weight', 'personality-assessment'); ?>" />
												<button class="button delete-label-button"><span
														class="dashicons dashicons-trash"></span></button>
											</div>
											<?php
										}
								...
								<button
									class="button add-label-button"><?php esc_html_e('Add Label', 'personality-assessment'); ?></button>
								<button class="button delete-answer-button" data-answer-id="<?php echo esc_attr($answer->id); ?>"
									data-nonce="<?php echo esc_attr(wp_create_nonce('pa_delete_answer_' . $answer->id)); ?>"><span
										class="dashicons dashicons-trash"></span></button>
							</div>
				...
				<button class="button add-answer-button" data-question-id="<?php echo esc_attr($question->id); ?>"
					data-nonce="<?php echo esc_attr(wp_create_nonce('pa_add_answer')); ?>"><?php esc_html_e('Add Answer', 'personality-assessment'); ?></button>
	...
	public function render_quizzes_list()
	{
		...
		<div class="wrap">
			...
			<a href="?page=personality-assessment&action=new"
				class="page-title-action"><?php esc_html_e('Add New', 'personality-assessment'); ?></a>
			<button id="pa-import-quiz-button"
				class="page-title-action"><?php esc_html_e('Import Quiz', 'personality-assessment'); ?></button>
			<hr class="wp-header-end">

			<div id="pa-import-form-wrapper" style="display: none; margin-bottom: 20px;">
				<form method="post" enctype="multipart/form-data">
					<input type="hidden" name="action" value="import_quiz_file" />
					<?php wp_nonce_field('pa_import_quiz_file', 'pa_import_quiz_nonce'); ?>
					<h2><?php esc_html_e('Import Quiz from CSV or JSON', 'personality-assessment'); ?></h2>
					<p><?php esc_html_e('Upload a CSV or JSON file to create a new quiz.', 'personality-assessment'); ?></p>
					<p>
						<label for="pa_quiz_file"><?php esc_html_e('Choose a file:', 'personality-assessment'); ?></label>
						<input type="file" id="pa_quiz_file" name="pa_quiz_file" accept=".csv,.json" required />
					</p>
	...
	public function render_result_details_page()
	{
		...
		<div class="wrap">
			...
			<a href="<?php echo esc_url(admin_url('admin.php?page=pa-results')); ?>"
				class="page-title-action"><?php esc_html_e('Back to Results', 'personality-assessment'); ?></a>
			<hr class="wp-header-end">
			<?php if (!$result): ?>
				<div class="notice notice-error">
					<p><?php esc_html_e('Invalid result specified.', 'personality-assessment'); ?></p>
				</div>
				<?php
				return;
			endif;
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $result->user_name ? esc_html($result->user_name) : esc_html__('Guest', 'personality-assessment'); ?>
						</td>
					</tr>
					<tr>
	...
	/**
	 * Import quiz from CSV or JSON.
	 */
	public function import_quiz_file()
	{
		if (!isset($_POST['action']) || 'import_quiz_file' !== $_POST['action']) {
			return;
		}

		if (!isset($_POST['pa_import_quiz_nonce']) || !wp_verify_nonce($_POST['pa_import_quiz_nonce'], 'pa_import_quiz_file')) {
			return;
		}
		...
		if (isset($_FILES['pa_quiz_file']) && UPLOAD_ERR_OK === $_FILES['pa_quiz_file']['error']) {
			$file = $_FILES['pa_quiz_file']['tmp_name'];
			...
			$file_name = sanitize_file_name($_FILES['pa_quiz_file']['name']);
			$file_ext = pathinfo($file_name, PATHINFO_EXTENSION);

			if ('json' === strtolower($file_ext)) {
				$this->import_quiz_from_json($file);
			} else {
				$this->import_quiz_from_csv($file, $file_name);
			}
	 ...
	 */
	private function import_quiz_from_json($file)
	{
		$json_content = file_get_contents($file);
		$data = json_decode($json_content, true);

		if (!$data || !isset($data['title'])) {
			wp_safe_redirect(add_query_arg(array('pa-import-error' => '3'), admin_url('admin.php?page=personality-assessment')));
			exit;
		}
		...
		$wpdb->insert(
			...
			array(
				...
				'slug' => sanitize_title($data['title']) . '-' . time(),
				'description' => isset($data['description']) ? sanitize_textarea_field($data['description']) : '',
				'status' => 'draft',
				'require_login' => isset($data['require_login']) ? intval($data['require_login']) : 1,
				'webhook_url' => isset($data['webhook_url']) ? sanitize_url($data['webhook_url']) : '',
				'webhook_secret' => isset($data['webhook_secret']) ? sanitize_text_field($data['webhook_secret']) : '',
				'success_message' => isset($data['success_message']) ? wp_kses_post($data['success_message']) : '',
			)
		);
		$quiz_id = $wpdb->insert_id;
			...
			foreach ($data['questions'] as $question_data) {
				$wpdb->insert(
					...
					array(
						...
						'type' => isset($question_data['type']) ? sanitize_text_field($question_data['type']) : 'single',
						'position' => $question_position++,
						'is_required' => isset($question_data['is_required']) ? intval($question_data['is_required']) : 1,
					)
				);
				$question_id = $wpdb->insert_id;
				...
				if (isset($question_data['answers']) && is_array($question_data['answers'])) {
					...
					foreach ($question_data['answers'] as $answer_data) {
						$label_weights = isset($answer_data['label_weights']) ? $answer_data['label_weights'] : array();
						$wpdb->insert(
							"{$wpdb->prefix}pa_answers",
							array(
								'question_id' => $question_id,
								'label' => sanitize_text_field($answer_data['label']),
	 ...
	 */
	private function import_quiz_from_csv($file, $file_name)
	{
	 ...
	 * Export quiz to JSON.
	 */
	public function export_quiz_json()
	{
		if (!current_user_can('manage_options')) {
			return;
		}

		if (!isset($_GET['_wpnonce']) || !wp_verify_nonce($_GET['_wpnonce'], 'pa_export_quiz_json')) {
			return;
		}
		...
		$export_data = array(
			...
			'webhook_secret' => $quiz->webhook_secret,
			'success_message' => $quiz->success_message,
			'questions' => array(),
		);
		...
		$filename = sanitize_title($quiz->title) . '.json';
